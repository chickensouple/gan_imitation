...
from tf_utils import fully_connected
import pdb
...
class ReplayBuffer(object):
    ...
    def put(self, state, action, reward, next_state, terminal):
...
class CriticModel(object):
    ...
    def q_vals_target(self, states, actions):
...
class DDPG(object):
    ...
    def curr_policy(self):
        ...
        # return partial(self.actor.get_action, sess=self.sess)
    ...
    def update(self, env, get_state):
        ...
        for i in range(max_iter):
            ...
            # if self.train_idx >= 200 * 50:
                sample = self.replay_buffer.sample(self.batch_size)

                # get target actions
                target_actions = self.actor.target_policy(sample['next_state'])
                target_q_vals = self.critic.q_vals_target(sample['next_state'], target_actions)
