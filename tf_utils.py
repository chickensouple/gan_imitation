...
# def conv_layer(input_var, filter_size, num_filters, activation=None, use_bias=True):
#   num_input = int(input_var.shape[3])
#   w = tf.Variable(tf.truncated_normal([filter_size, filter_size, num_input, num_filters]), name='w')

#   temp = tf.nn.conv2d(input_var, w, strides=[1, 1, 1, 1], padding='SAME')
#   if use_bias:
